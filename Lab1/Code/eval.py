...
for i in range(len(cards)):
    ...
    for j in range(0, n_samples_per_card, batch_size):
        ...
        x_batch = [X_test[i][k] for k in range(j, min(j+batch_size, n_samples_per_card))]
        x_batch = torch.nn.utils.rnn.pad_sequence(
            [torch.tensor(np.array(x, dtype=np.int64), dtype=torch.long) for x in x_batch],
...
plt.subplot(1, 2, 1)
plt.plot(results['deepsets']['acc'], label='DeepSets')
plt.plot(results['lstm']['acc'], label='LSTM')
plt.title('Accuracy')
...
plt.subplot(1, 2, 2)
plt.plot(results['deepsets']['mae'], label='DeepSets')
plt.plot(results['lstm']['mae'], label='LSTM')
